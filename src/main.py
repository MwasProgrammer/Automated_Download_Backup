...
def main():
    ...
    try:
        ...
        logger.info(f"Starting thread pool to process {batch_limit} files ...")
        max_workers = min(4, os.cpu_count() or 2) 

        with ThreadPoolExecutor(max_workers = max_workers) as thread_pool:
            future_to_file = {
                thread_pool.submit(process_single_file, file, configuration, backup_root, db, logger):file
                for file in batch_limit and files_to_process[:batch_limit] #
            } 
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    if result == BackupStatus.BACKUP_SUCCESS:
                        successful_backedup_files.append(file_obj.name)

                except Exception as e:
                    logger.error(f"Error processing file {file_obj.name}: {e}")
                    dashboard_stats[BackupStatus.FAILED] += 1
